        ...
        for i in range(1, 4):
            for j in range(4):
                ...
                if i - shift - 1 >= 0:  # 범위를 벗어 나지 않고
                    ...
                    merged[i - shift][j]:  # 다 움직인 후에 위의 블록과 합칠 수 있으면 합침.
                        board[i - shift - 1][j] *= 2
        ...
        for i in range(2, -1, -1):
            for j in range(4):
                ...
                if i + shift + 1 <= 3:  # 범위를 벗어 나지 않고
                    if board[i + shift][j] == board[i + shift + 1][j] and not merged[i + shift + 1][j] and not \
                    merged[i + shift][j]:  # 다 움직인 후에 위의 블록과 합칠 수 있으면 합침.
                        board[i + shift + 1][j] *= 2
                        score += board[i + shift + 1][j]  # 합친 수만큼 점수 획득
                        board[i + shift][j] = 0
        ...
        for i in range(4):
            for j in range(1, 4):
                ...
                if j - shift - 1 >= 0:  # 범위를 벗어 나지 않고
                    ...
                    merged[i][j - shift]:  # 다 움직인 후에 위의 블록과 합칠 수 있으면 합침.
                        board[i][j - shift - 1] *= 2
        ...
        for i in range(4):
            for j in range(2, -1, -1):
                ...
                if j + shift + 1 <= 3:  # 범위를 벗어 나지 않고
                    if board[i][j + shift + 1] == board[i][j + shift] and not merged[i][j + shift + 1] and not \
                    merged[i][j + shift]:  # 다 움직인 후에 위의 블록과 합칠 수 있으면 합침.
                        board[i][j + shift + 1] *= 2
                        score += board[i][j + shift + 1]  # 합친 수만큼 점수 획득
                        board[i][j + shift] = 0
